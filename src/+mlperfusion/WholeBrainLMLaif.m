classdef WholeBrainLMLaif < mlperfusion.IMRCurve & mlperfusion.ILaif  
    ...
    methods (Static)
        function this   = load(fname, varargin)
            %% LOAD
            %  Usage:   this = WholeBrainLMLaif.load(perfusion4dfp_logfile[, parameter_name, parameter_value])
            
            this = mlperfusion.WholeBrainLMLaif(fname, varargin{:});
        end 
    end
    
	methods 		  
 		function this = WholeBrainLMLaif(perfLoc, wholeBrainDSC, varargin) 
 			%% WHOLEBRAINLMLAIF 
 			%  Usage:  this = WholeBrainLMLaif(perfusion4dfp_logfile, WholeBrainDSC_object[, parameter_name, parameter_value]) 
            ...
            this.perfusionIO_ = mlperfusion.PerfusionIO.load(p.Results.perfLoc);
        ...
        function saveas(~)
            error('mlperfusion:notImplemented', 'WholeBrainLMLaif.saveas');
        end
        function save(~)
            error('mlperfusion:notImplemented', 'WholeBrainLMLaif.save');
        end
    end 
